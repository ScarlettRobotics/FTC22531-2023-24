...
@Autonomous(name = "BlueBackdropNear", group = "blue")
public class BlueBackdropNear extends LinearOpMode {
    // Timing related
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if (eventManager.eventOccurred(timer.time(), 0)) {
                drivetrainCore.forwardByEncoder(-850);
            }

            if (eventManager.eventOccurred(timer.time(), 1)) {
                drivetrainCore.rotateByEncoder(510);
            }

            if (eventManager.eventOccurred(timer.time(), 2)) {
                drivetrainCore.forwardByEncoder(-1800);
                armCore.setTargetPosition(-1900);
            }
