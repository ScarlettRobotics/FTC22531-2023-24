        ...
        while (opModeIsActive()) {
            ...
            if (eventManager.eventOccurred(timer.time(), 0)) {
                if (!tensorFlowCore.recognizing()) { // not recognizing any cubes
                    propLocation = 0;
                    drivetrainCore.moveByEncoder(500, 500);
                } else if (tensorFlowCore.getX() > (double) 640 /2) { // to left of camera
                    propLocation = 1;
                    drivetrainCore.moveByEncoder(700, 700);
                } else { // to right of camera
                    propLocation = 2;
                    drivetrainCore.moveByEncoder(400, 400);
                }
            ...
            if (eventManager.eventOccurred(timer.time(), 1)) {
                if (propLocation ==  0) {
                    drivetrainCore.moveByEncoder(325, 325);
                } else if (propLocation == 2) {
                    drivetrainCore.moveByEncoder(-180, -180);
                }
            ...
            if (eventManager.eventOccurred(timer.time(), 5)) {
                if (propLocation == 0) {
                    drivetrainCore.moveByEncoder(-925, -925);
                } else if (propLocation == 2) {
                    drivetrainCore.moveByEncoder(-470, -470);
                }
            ...
            if (eventManager.eventOccurred(timer.time(), 7)) {
                drivetrainCore.moveByEncoder(1100, 1100);
